...
for (i in mvlHGVS_gemROW[!is.na(mvlHGVS_gemROW$match),]$i){
  if (!is.na(match) & (mvl_excel %>% filter(HGVS==i) %>% nrow)==1){
    gem[i,'Priority'] <- mvl_excel %>% filter(HGVS==i) %>% pull(Priority)
    gem[i,'HGVS'] <- mvl_excel %>% filter(HGVS==i) %>% pull(HGVS)
    gem[i,'TimesObservedPerPanel'] <- mvl_excel %>% filter(HGVS==i) %>% pull(TimesObservedPerPanel)
    gem[i,'TimesObservedPerPanelGroup'] <- mvl_excel %>% filter(HGVS==i) %>% pull(TimesObservedPerPanelGroup)
  }
}
...
times_panel <- grep('TimesObservedPerPanel$', colnames(gem_view), ignore.case = T)
group_panel <- grep('TimesObservedPerPanelGroup', colnames(gem_view), ignore.case = T)
# scholar
...
out <- gem_view %>% 
  dplyr::filter(Priority == 'Seoncdary' | is.na(Priority)) %>% 
  dplyr::arrange(DeleteriousMark, `impact severity`) %>% 
  DT::datatable(width=1200, 
                escape = FALSE,
                rownames = F, 
