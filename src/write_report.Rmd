...
if (length(HGVS[!HGVS %in% all_DF$vcf_id]) > 0){
  for (i in HGVS[!HGVS %in% all_DF$vcf_id]){
    ...
    if (length(query_result) != 2) {
      # if multiple results returned, just take first one
      query_result <- query_result[c(1,2)]
    }
...
all_DF_view$clinvar_sig = apply(all_DF_view, 1, function(x) clinvar_link_generator('https://www.ncbi.nlm.nih.gov/clinvar?term=',as.character(x['clinvar_id']), as.character(x['clinvar_sig'])))
