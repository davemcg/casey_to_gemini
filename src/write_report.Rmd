...
gem <- fread(params$gemini_return) %>% mutate(chrom=as.character(chrom))
mvl_excel <- fread(params$patient_MVL_excel, header = F)
colnames(mvl_excel) <- c('HGVS','Priority','Rs_ids', 'ChromPos', 'Ref', 'Alt', 'TimesObservedPerPanel', 'TimesObservedPerPanelGroup')
mvl_excel <- mvl_excel %>% separate(ChromPos, c('chrom','end')) %>% mutate(chrom=as.character(chrom), ref=Ref, alt=Alt, end=as.numeric(end))

# join gemini annotated data with mvl info by chrom pos ref alt
gem <- gem %>% left_join(., mvl_excel, by = c('chrom','end','ref','alt'))

# match failures by rs id
gem_noMatch <- gem %>% filter(is.na(Priority))
mvl_noMatch <- mvl_excel %>% filter(!HGVS %in% gem$HGVS)

more_matched <-  left_join(gem_noMatch %>% select(chrom:hom_alt_samples), mvl_noMatch %>% select(HGVS, Priority, TimesObservedPerPanel, TimesObservedPerPanelGroup, Rs_ids), by=c('rs_ids'="Rs_ids"))

# increase gem info with rs matches
gem <- bind_rows(gem %>% filter(!is.na(Priority)), more_matched)

# match remaining values with HGVS

# match remaining failures by HGVS
...
mvlHGVS_gemROW <- data.frame()
for (i in mvl_noMatch$HGVS){
  match <- hgvs_matcher(i, gem)
  if (length(match)==0) {
    match <- NA
  }
  mvlHGVS_gemROW <- rbind(mvlHGVS_gemROW, cbind(i, match))
}

for (hgvs in mvlHGVS_gemROW[!is.na(mvlHGVS_gemROW$match),]$i){
  if (!is.na(match) & (mvl_excel %>% filter(HGVS==hgvs) %>% nrow)==1){
    gem[mvlHGVS_gemROW %>% filter(i==hgvs) %>% pull(match) %>% as.character() %>% as.numeric(),'Priority'] <- mvl_excel %>% filter(HGVS==hgvs) %>% pull(Priority)
    gem[mvlHGVS_gemROW %>% filter(i==hgvs) %>% pull(match) %>% as.character() %>% as.numeric(),'HGVS'] <- mvl_excel %>% filter(HGVS==hgvs) %>% pull(HGVS)
    gem[mvlHGVS_gemROW %>% filter(i==hgvs) %>% pull(match) %>% as.character() %>% as.numeric(),'TimesObservedPerPanel'] <- mvl_excel %>% filter(HGVS==hgvs) %>% pull(TimesObservedPerPanel)
    gem[mvlHGVS_gemROW %>% filter(i==hgvs) %>% pull(match) %>% as.character() %>% as.numeric(),'TimesObservedPerPanelGroup'] <- mvl_excel %>% filter(HGVS==hgvs) %>% pull(TimesObservedPerPanelGroup)
  }
}

mvl_not_ever_match <- mvl_excel %>% filter(!HGVS %in% gem$HGVS)
```
...
# prep for printing
gem_view <- gem %>% mutate(ID=paste(chrom,end,ref,alt,sep='-')) %>% select(ID, Genotype, vcf_id:hom_alt_samples, Priority, TimesObservedPerPanel, TimesObservedPerPanelGroup) %>% mutate_if(is.factor, as.character) %>% data.table()
