...
all_DF_view <- all_DF_view %>% 
  dplyr::mutate(clinvar_diseases = gsub('\\|',' ', as.character(clinvar_diseases)),
                ...
                DeleteriousMark = ifelse((impact_severity=='HIGH' | grepl('pathog', clinvar_sig)) & max_aaf_all < 0.1, 'Candidate', NA),
                impact_severity = factor(impact_severity, levels = c('HIGH','MED','LOW',NA))) 

all_DF_view$hvgsp = strsplit(as.character(all_DF_view$hgvsp),':')[[1]][2]
all_DF_view$ID = link_generator('http://gnomad.broadinstitute.org/variant/',as.character(all_DF_view$ID))
all_DF_view$clinvar_sig = clinvar_link_generator('https://www.ncbi.nlm.nih.gov/clinvar?term=',as.character(all_DF_view$rs_ids), as.character(all_DF_view$clinvar_sig))
all_DF_view$rs_ids=link_generator('https://www.ncbi.nlm.nih.gov/projects/SNP/snp_ref.cgi?rs=',as.character(all_DF_view$rs_ids))
all_DF_view$'Google Scholar' = google_link_generator(as.character(all_DF_view$vcf_id), as.character((all_DF_view$gene)))
all_DF_view$gene = link_generator('https://www.omim.org/search/?search=', as.character(all_DF_view$gene))
all_DF_view$vcf_id = link_generator('https://www.mutalyzer.nl/name-checker?description=',as.character(all_DF_view$vcf_id))
all_DF_view <- all_DF_view %>% 
...
core <- c(1, gts_index, 3,5,7,8,9,10,15, het_index, hom_index, gs) 
...
if (nrow(all_DF_view %>% filter(Priority == 'Primary')) == 0){
  ...
  out <- all_DF_view %>% 
    ...
    DT::formatSignif(columns=c('af exac afr','af exac amr','af exac eas','af exac oth','af exac sas','an exac all','mis z','pli'),3) %>% 
