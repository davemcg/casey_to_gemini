...
all_DF_view <- all_DF_view %>% 
  dplyr::mutate(clinvar_diseases = gsub('\\|',' ', as.character(clinvar_diseases)),
                ...
                DeleteriousMark = ifelse(impact_severity=='HIGH' | grepl('pathog', clinvar_sig), 'Candidate', NA),
...
if (nrow(all_DF_view %>% filter(Priority == 'Primary')) == 0){
  ...
  out <- all_DF_view %>% 
    ...
    DT::formatStyle(columns = c('DeleteriousMark'), target = 'row', backgroundColor = DT::styleEqual(c(NA, 'Candidate'), c('white', 'lightred')))
}
out
```
