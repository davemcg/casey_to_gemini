...
## Primary Variants 
```{r}
all_DF_view <- all_DF %>% mutate(ID=paste(chrom,end,ref,alt,sep='-')) %>% select(ID, vcf_id:Priority) %>% bind_rows(failed)
# find gt index
gts_index <- grep('gts',colnames(all_DF_view))
colnames(all_DF_view)[gts_index] <- gsub('_', ' ', sample_name)
...
if (nrow(all_DF_view %>% filter(Priority == 'Primary'))>0){
  all_DF_view %>% 
    ...
    rowwise() %>% 
    mutate(hgvsp = strsplit(as.character(hgvsp),':')[[1]][2],
...
## Secondary/benign/hidden Variants
```{r}
if (nrow(all_DF_view %>% filter(Priority == 'Primary'))>0){
  all_DF_view %>% 
    filter(Priority == 'Primary') %>% 
