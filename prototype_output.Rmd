...
# prep for printing
all_DF_view <- all_DF %>% mutate(ID=paste(chrom,end,ref,alt,sep='-')) %>% select(ID, Genotype, vcf_id:Priority) %>% bind_rows(failed) %>% mutate_if(is.factor, as.character)
# format DF
...
exac <- c(3, gts_index, 7, grep('exac', colnames(all_DF_view), ignore.case = T))
neg_exac <- setdiff(all_cols, exac) - 1
core <- c(1, gts_index, 3,5,7,8,9,15, het_index, hom_index, gs)
