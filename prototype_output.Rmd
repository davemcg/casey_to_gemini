...
all_DF_view <- all_DF_view %>% 
  dplyr::mutate(clinvar_diseases = gsub('\\|',' ', as.character(clinvar_diseases)),
                ...
                DeleteriousMark = factor(ifelse(impact_severity=='HIGH' | grepl('pathog', clinvar_sig), 'Candidate', NA), levels=c('HIGH','MED','LOW'))) %>% 
  dplyr::rowwise() %>% 
  dplyr::mutate(hgvsp = strsplit(as.character(hgvsp),':')[[1]][2],
                ...
                gene = link_generator('https://www.omim.org/search/?search=', as.character(gene)),
                vcf_id = link_generator('https://www.mutalyzer.nl/name-checker?description=',as.character(vcf_id))) %>% 
  dplyr::arrange(DeleteriousMark) %>% 
  data.table()
...
core <- c(1, gts_index, 3,5,7,8,9,10,14, het_index, hom_index, gs) 
...
if (nrow(all_DF_view %>% filter(Priority == 'Primary')) == 0){
  ...
  out <- all_DF_view %>% 
    ...
    DT::formatStyle(columns = c('DeleteriousMark'), target = 'row', backgroundColor = DT::styleEqual(c(NA, 'Candidate'), c('white', 'brown1')))
}
out
```
...
This report is auto-generated with scripts written by David McGaughey on `r strftime(Sys.time(), format = "%B %d, %Y")`
